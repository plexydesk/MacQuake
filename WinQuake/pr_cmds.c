...
void PF_ftos (void)
{
	...
	if (v == (int)v)
		...
		sprintf (temp, "%5.1f",v);
	G_INT(OFS_RETURN) = PR_SetString(temp);
}
...
{
	char temp[128];
	sprintf (temp, "'%5.1f %5.1f %5.1f'", G_VECTOR(OFS_PARM0)[0], G_VECTOR(OFS_PARM0)[1], G_VECTOR(OFS_PARM0)[2]);
	G_INT(OFS_RETURN) = PR_SetString(temp);
}
...
{
	char temp[128];
	sprintf (temp, "entity %i", G_EDICTNUM(OFS_PARM0));
	G_INT(OFS_RETURN) = PR_SetString(temp);
}
#endif
